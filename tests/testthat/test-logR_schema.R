...
test_that("schema created properly", {
  ...
  logR_schema("h2")
  seq_dt <- setDT(dbGetQuery(getOption("logR.conn"), "SELECT sequence_name, current_value FROM INFORMATION_SCHEMA.SEQUENCES"))
  tblv_dt <- setDT(dbGetQuery(getOption("logR.conn"), "SELECT table_type, table_name FROM INFORMATION_SCHEMA.TABLES WHERE table_schema != 'INFORMATION_SCHEMA'"))
