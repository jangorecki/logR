...
The logR uses databases unification layer served by imported package [dwtools](https://github.com/jangorecki/dwtools). Database connection has to be defined according to `db` function in `dwtools` package. When connections are defined you just need to point the name of connection which will be used for storing logR logs. In the vignette we will use H2 database because it does not require heavy external dependencies. You can also log to csv file instead of database but logging to csv does not support transactional logging.
...
We defined single database connection, we set logR to use database and to use connection named *h2*.  
For the first time we need to create database objects which are required for logR processing: **sequence**, **view** and a **table**.  
Function `logR_schema()` will handle creation of those objects for some databases, you can get plain sql scripts from `schema_sql()` function and adjust it to your database. If your database does not support sequences you may need to simulate a sequence behind the view.
